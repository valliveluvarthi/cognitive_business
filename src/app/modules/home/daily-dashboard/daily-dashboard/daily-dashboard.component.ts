...
import { faArrowLeft, faCalendarDay, faCheckCircle, faChevronDown, faExclamationCircle, faExclamationTriangle, faExternalLinkAlt, faPauseCircle, faPlayCircle, faTimes } from '@fortawesome/free-solid-svg-icons';
  ...
  */ 
  loadPageData() {
    ...
    this.decisionService.loadPageData(this.selectedSite.key, this.selectedTurbine.key, this.selectedPeriod.key, "daily").subscribe( async (response) =>  {
      ...
      console.log(" this.selectedTurbineSignal ", this.selectedTurbineSignal); 
    ...
    for (let i = 0; i < data.columns.length; i++) {
      ...
      if (element == 'Now') {
        for (let j = 0; j < data.rows.length; j++) {
          ...
          console.log("rowData: ", rowData, " i ",i);
          obj[rowData.title] = {
            item: rowData.data[i]
          };
        }
